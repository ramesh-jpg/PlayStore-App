...
public class UserRepositoryImpl implements UserRepository{
    ...
    public void save(final User user) {
        try (final Connection connection = ConnectionUtil.getConnection();
             final PreparedStatement statement = connection.prepareStatement("INSERT INTO users(username,password,email,phone,role) VALUES (?,?,?,?,?)")) {
    ...
    public User getByUsername(final String username) {
        try (final Connection connection = ConnectionUtil.getConnection();
            ...
            if (result.next()) return new User(result.getInt(1), result.getString(2), result.getString(3), null, 0L, result.getString(6));
